...
#include "ConvFnc_BandpassBiQ_FiP16.h"
...
/** Private prototypes */
static uint8 convertM2I(const BANDPASSBIQ_FIP16_MASK_PARAM *maskParam, BANDPASSBIQ_FIP16_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 BandpassBiQ_FiP16_SaveMP(BANDPASSBIQ_FIP16 *block, BANDPASSBIQ_FIP16_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        BandpassBiQ_FiP16_BackupMP(maskParam);
        ...
        error = BandpassBiQ_FiP16_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            BandpassBiQ_FiP16_RestoreMP(maskParam);
        }
...
uint8 BandpassBiQ_FiP16_InitMP(BANDPASSBIQ_FIP16 *block, const BANDPASSBIQ_FIP16_MASK_PARAM *maskParam)
{
    ...
    BANDPASSBIQ_FIP16_IMPL_PARAM *implParam = (BANDPASSBIQ_FIP16_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = BandpassBiQ_FiP16_Save(block, SaveFncDataBuffer, 15);
