...
#include "ConvFnc_Biquad_Float64.h"
...
typedef struct {
    ...
    float64 a1;
    float64 a2;
} BIQUAD_FLOAT64_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const BIQUAD_FLOAT64_MASK_PARAM *maskParam, BIQUAD_FLOAT64_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 Biquad_Float64_SaveMP(BIQUAD_FLOAT64 *block, BIQUAD_FLOAT64_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        Biquad_Float64_BackupMP(maskParam);
        ...
        error = Biquad_Float64_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            Biquad_Float64_RestoreMP(maskParam);
        }
...
uint8 Biquad_Float64_InitMP(BIQUAD_FLOAT64 *block, const BIQUAD_FLOAT64_MASK_PARAM *maskParam)
{
    ...
    BIQUAD_FLOAT64_IMPL_PARAM *implParam = (BIQUAD_FLOAT64_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = Biquad_Float64_Save(block, SaveFncDataBuffer, 40);
