...
#include "ConvFnc_BandstopBiQ_Float32.h"
...
typedef struct {
    ...
    float32 a1;
    float32 a2;
} BANDSTOPBIQ_FLOAT32_IMPL_PARAM;
...
/** Private prototypes */
static uint8 convertM2I(const BANDSTOPBIQ_FLOAT32_MASK_PARAM *maskParam, BANDSTOPBIQ_FLOAT32_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 BandstopBiQ_Float32_SaveMP(BANDSTOPBIQ_FLOAT32 *block, BANDSTOPBIQ_FLOAT32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        BandstopBiQ_Float32_BackupMP(maskParam);
        ...
        error = BandstopBiQ_Float32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            BandstopBiQ_Float32_RestoreMP(maskParam);
        }
...
uint8 BandstopBiQ_Float32_InitMP(BANDSTOPBIQ_FLOAT32 *block, const BANDSTOPBIQ_FLOAT32_MASK_PARAM *maskParam)
{
    ...
    BANDSTOPBIQ_FLOAT32_IMPL_PARAM *implParam = (BANDSTOPBIQ_FLOAT32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = BandstopBiQ_Float32_Save(block, SaveFncDataBuffer, 20);
