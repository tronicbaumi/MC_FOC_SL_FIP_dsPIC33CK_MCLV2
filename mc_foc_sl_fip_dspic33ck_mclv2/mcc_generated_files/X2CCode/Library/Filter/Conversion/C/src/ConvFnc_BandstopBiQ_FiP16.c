...
#include "ConvFnc_BandstopBiQ_FiP16.h"
...
/** Private prototypes */
static uint8 convertM2I(const BANDSTOPBIQ_FIP16_MASK_PARAM *maskParam, BANDSTOPBIQ_FIP16_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 BandstopBiQ_FiP16_SaveMP(BANDSTOPBIQ_FIP16 *block, BANDSTOPBIQ_FIP16_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        BandstopBiQ_FiP16_BackupMP(maskParam);
        ...
        error = BandstopBiQ_FiP16_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            BandstopBiQ_FiP16_RestoreMP(maskParam);
        }
...
uint8 BandstopBiQ_FiP16_InitMP(BANDSTOPBIQ_FIP16 *block, const BANDSTOPBIQ_FIP16_MASK_PARAM *maskParam)
{
    ...
    BANDSTOPBIQ_FIP16_IMPL_PARAM *implParam = (BANDSTOPBIQ_FIP16_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = BandstopBiQ_FiP16_Save(block, SaveFncDataBuffer, 15);
