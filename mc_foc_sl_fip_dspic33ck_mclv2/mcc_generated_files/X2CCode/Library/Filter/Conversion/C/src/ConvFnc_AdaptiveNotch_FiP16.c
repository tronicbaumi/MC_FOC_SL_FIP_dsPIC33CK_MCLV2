...
#include "ConvFnc_AdaptiveNotch_FiP16.h"
...
typedef struct {
    ...
    int16 c_fact;
    int8 sfrA;
} ADAPTIVENOTCH_FIP16_IMPL_PARAM;
...
/** Private prototypes */
static uint8 convertM2I(const ADAPTIVENOTCH_FIP16_MASK_PARAM *maskParam, ADAPTIVENOTCH_FIP16_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    {    
        ...
        impParam->n_thresh = getQx16Value(n_thresh / n_max, BITS - 1);
        impParam->sfrA = sfrA;
        impParam->a_fact = getQx16Value(a_fact, sfrA);
        impParam->b_fact = getQx16Value(b_fact, BITS - 1);
        impParam->c_fact = getQx16Value(c_fact, BITS - 1);
    }
...
uint8 AdaptiveNotch_FiP16_SaveMP(ADAPTIVENOTCH_FIP16 *block, ADAPTIVENOTCH_FIP16_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        AdaptiveNotch_FiP16_BackupMP(maskParam);
        ...
        error = AdaptiveNotch_FiP16_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            AdaptiveNotch_FiP16_RestoreMP(maskParam);
        }
...
uint8 AdaptiveNotch_FiP16_InitMP(ADAPTIVENOTCH_FIP16 *block, const ADAPTIVENOTCH_FIP16_MASK_PARAM *maskParam)
{
    ...
    ADAPTIVENOTCH_FIP16_IMPL_PARAM *implParam = (ADAPTIVENOTCH_FIP16_IMPL_PARAM*)ImplParamBuffer;
    ...
    SaveFncDataBuffer[0] = (uint8)(implParam->n_thresh & 0x00FF);
    SaveFncDataBuffer[1] = (uint8)((implParam->n_thresh >> 8) & 0x00FF);
    SaveFncDataBuffer[2] = (uint8)(implParam->a_fact & 0x00FF);
    SaveFncDataBuffer[3] = (uint8)((implParam->a_fact >> 8) & 0x00FF);
    SaveFncDataBuffer[4] = (uint8)(implParam->b_fact & 0x00FF);
    SaveFncDataBuffer[5] = (uint8)((implParam->b_fact >> 8) & 0x00FF);
    SaveFncDataBuffer[6] = (uint8)(implParam->c_fact & 0x00FF);
    SaveFncDataBuffer[7] = (uint8)((implParam->c_fact >> 8) & 0x00FF);
    SaveFncDataBuffer[8] = (uint8)implParam->sfrA;

    /** - Execute save function of block */
    error = AdaptiveNotch_FiP16_Save(block, SaveFncDataBuffer, 9);
