...
#include "ConvFnc_BandpassBiQ_FiP32.h"
...
/** Private prototypes */
static uint8 convertM2I(const BANDPASSBIQ_FIP32_MASK_PARAM *maskParam, BANDPASSBIQ_FIP32_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 BandpassBiQ_FiP32_SaveMP(BANDPASSBIQ_FIP32 *block, BANDPASSBIQ_FIP32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        BandpassBiQ_FiP32_BackupMP(maskParam);
        ...
        error = BandpassBiQ_FiP32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            BandpassBiQ_FiP32_RestoreMP(maskParam);
        }
...
uint8 BandpassBiQ_FiP32_InitMP(BANDPASSBIQ_FIP32 *block, const BANDPASSBIQ_FIP32_MASK_PARAM *maskParam)
{
    ...
    BANDPASSBIQ_FIP32_IMPL_PARAM *implParam = (BANDPASSBIQ_FIP32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = BandpassBiQ_FiP32_Save(block, SaveFncDataBuffer, 25);
