...
#include "ConvFnc_Biquad_FiP16.h"
...
typedef struct {
    ...
    int8 sfra1;
    int8 sfra2;
} BIQUAD_FIP16_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const BIQUAD_FIP16_MASK_PARAM *maskParam, BIQUAD_FIP16_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    int8 sfrb0, sfrb1, sfrb2, sfra1, sfra2;
    ...
    else
    {
        ...
        sfrb2 = getQFormat(b2, BITS);
        sfra1 = getQFormat(a1, BITS);
        sfra2 = getQFormat(a2, BITS);
        ...
        impParam->b2 = getQx16Value(b2, sfrb2);
        impParam->a1 = getQx16Value(a1, sfra1);
        impParam->a2 = getQx16Value(a2, sfra2);
    }
...
uint8 Biquad_FiP16_SaveMP(BIQUAD_FIP16 *block, BIQUAD_FIP16_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        Biquad_FiP16_BackupMP(maskParam);
        ...
        error = Biquad_FiP16_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            Biquad_FiP16_RestoreMP(maskParam);
        }
...
uint8 Biquad_FiP16_InitMP(BIQUAD_FIP16 *block, const BIQUAD_FIP16_MASK_PARAM *maskParam)
{
    ...
    BIQUAD_FIP16_IMPL_PARAM *implParam = (BIQUAD_FIP16_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = Biquad_FiP16_Save(block, SaveFncDataBuffer, 15);
