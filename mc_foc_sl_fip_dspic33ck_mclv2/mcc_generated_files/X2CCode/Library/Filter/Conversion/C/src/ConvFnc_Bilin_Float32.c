...
#include "ConvFnc_Bilin_Float32.h"
...
typedef struct {
    ...
    float32 b1;
    float32 a1;
} BILIN_FLOAT32_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const BILIN_FLOAT32_MASK_PARAM *maskParam, BILIN_FLOAT32_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 Bilin_Float32_SaveMP(BILIN_FLOAT32 *block, BILIN_FLOAT32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        Bilin_Float32_BackupMP(maskParam);
        ...
        error = Bilin_Float32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            Bilin_Float32_RestoreMP(maskParam);
        }
...
uint8 Bilin_Float32_InitMP(BILIN_FLOAT32 *block, const BILIN_FLOAT32_MASK_PARAM *maskParam)
{
    ...
    BILIN_FLOAT32_IMPL_PARAM *implParam = (BILIN_FLOAT32_IMPL_PARAM*)ImplParamBuffer;
    ...
    SaveFncDataBuffer[8] = (uint8)((*(uint32*)&(implParam->a1)) & 0x000000FF);
    SaveFncDataBuffer[9] = (uint8)((*(uint32*)&(implParam->a1) >> 8) & 0x000000FF);
    SaveFncDataBuffer[10] = (uint8)((*(uint32*)&(implParam->a1) >> 16) & 0x000000FF);
    SaveFncDataBuffer[11] = (uint8)((*(uint32*)&(implParam->a1) >> 24) & 0x000000FF);

    /** - Execute save function of block */
    error = Bilin_Float32_Save(block, SaveFncDataBuffer, 12);
