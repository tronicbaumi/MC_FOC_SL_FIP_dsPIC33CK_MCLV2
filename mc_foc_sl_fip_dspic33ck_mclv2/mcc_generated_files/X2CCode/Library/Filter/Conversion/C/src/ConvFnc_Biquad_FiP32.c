...
#include "ConvFnc_Biquad_FiP32.h"
...
typedef struct {
    ...
    int8 sfra1;
    int8 sfra2;
} BIQUAD_FIP32_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const BIQUAD_FIP32_MASK_PARAM *maskParam, BIQUAD_FIP32_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 Biquad_FiP32_SaveMP(BIQUAD_FIP32 *block, BIQUAD_FIP32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        Biquad_FiP32_BackupMP(maskParam);
        ...
        error = Biquad_FiP32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            Biquad_FiP32_RestoreMP(maskParam);
        }
...
uint8 Biquad_FiP32_InitMP(BIQUAD_FIP32 *block, const BIQUAD_FIP32_MASK_PARAM *maskParam)
{
    ...
    BIQUAD_FIP32_IMPL_PARAM *implParam = (BIQUAD_FIP32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = Biquad_FiP32_Save(block, SaveFncDataBuffer, 25);
