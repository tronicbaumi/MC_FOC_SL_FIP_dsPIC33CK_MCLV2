...
#include "ConvFnc_HighpassBiQ_Float64.h"
...
typedef struct {
    ...
    float64 a1;
    float64 a2;
} HIGHPASSBIQ_FLOAT64_IMPL_PARAM;
...
/** Private prototypes */
static uint8 convertM2I(const HIGHPASSBIQ_FLOAT64_MASK_PARAM *maskParam, HIGHPASSBIQ_FLOAT64_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 HighpassBiQ_Float64_SaveMP(HIGHPASSBIQ_FLOAT64 *block, HIGHPASSBIQ_FLOAT64_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        HighpassBiQ_Float64_BackupMP(maskParam);
        ...
        error = HighpassBiQ_Float64_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            HighpassBiQ_Float64_RestoreMP(maskParam);
        }
...
uint8 HighpassBiQ_Float64_InitMP(HIGHPASSBIQ_FLOAT64 *block, const HIGHPASSBIQ_FLOAT64_MASK_PARAM *maskParam)
{
    ...
    HIGHPASSBIQ_FLOAT64_IMPL_PARAM *implParam = (HIGHPASSBIQ_FLOAT64_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = HighpassBiQ_Float64_Save(block, SaveFncDataBuffer, 40);
