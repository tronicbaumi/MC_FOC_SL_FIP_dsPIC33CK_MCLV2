...
#include "ConvFnc_AdaptiveNotch_FiP32.h"
...
typedef struct {
    ...
    int32 c_fact;
    int8 sfrA;
} ADAPTIVENOTCH_FIP32_IMPL_PARAM;
...
/** Private prototypes */
static uint8 convertM2I(const ADAPTIVENOTCH_FIP32_MASK_PARAM *maskParam, ADAPTIVENOTCH_FIP32_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    float_CoT n_max = getAbsValR64(maskParam->n_max);   /* speed scaling */
    ...
    if ((((2 * n_max) / 60) * p) > (1 / ts)) {
        error += 8; /* Nyquist criteria is violated -> decrease n_max or Ts*ts_fact! */
    }
    ...
    {    
        ...
        int8 sfrA = getQFormat(a_fact, BITS);
        /* B and C factors are always smaller than 1 -> no shift factor calculation needed */
        
        /* set implementation parameter values */
        impParam->n_thresh = getQx32Value(n_thresh / n_max, BITS - 1);
        impParam->sfrA = sfrA;
        impParam->a_fact = getQx32Value(a_fact, sfrA);
        impParam->b_fact = getQx32Value(b_fact, BITS - 1);
        impParam->c_fact = getQx32Value(c_fact, BITS - 1);
    }
...
uint8 AdaptiveNotch_FiP32_SaveMP(ADAPTIVENOTCH_FIP32 *block, ADAPTIVENOTCH_FIP32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        AdaptiveNotch_FiP32_BackupMP(maskParam);
        ...
        error = AdaptiveNotch_FiP32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            AdaptiveNotch_FiP32_RestoreMP(maskParam);
        }
...
uint8 AdaptiveNotch_FiP32_InitMP(ADAPTIVENOTCH_FIP32 *block, const ADAPTIVENOTCH_FIP32_MASK_PARAM *maskParam)
{
    ...
    ADAPTIVENOTCH_FIP32_IMPL_PARAM *implParam = (ADAPTIVENOTCH_FIP32_IMPL_PARAM*)ImplParamBuffer;
    ...
    SaveFncDataBuffer[0] = (uint8)(implParam->n_thresh & 0x000000FF);
    SaveFncDataBuffer[1] = (uint8)((implParam->n_thresh >> 8) & 0x000000FF);
    SaveFncDataBuffer[2] = (uint8)((implParam->n_thresh >> 16) & 0x000000FF);
    SaveFncDataBuffer[3] = (uint8)((implParam->n_thresh >> 24) & 0x000000FF);
    SaveFncDataBuffer[4] = (uint8)(implParam->a_fact & 0x000000FF);
    SaveFncDataBuffer[5] = (uint8)((implParam->a_fact >> 8) & 0x000000FF);
    SaveFncDataBuffer[6] = (uint8)((implParam->a_fact >> 16) & 0x000000FF);
    SaveFncDataBuffer[7] = (uint8)((implParam->a_fact >> 24) & 0x000000FF);
    SaveFncDataBuffer[8] = (uint8)(implParam->b_fact & 0x000000FF);
    SaveFncDataBuffer[9] = (uint8)((implParam->b_fact >> 8) & 0x000000FF);
    SaveFncDataBuffer[10] = (uint8)((implParam->b_fact >> 16) & 0x000000FF);
    SaveFncDataBuffer[11] = (uint8)((implParam->b_fact >> 24) & 0x000000FF);
    SaveFncDataBuffer[12] = (uint8)(implParam->c_fact & 0x000000FF);
    SaveFncDataBuffer[13] = (uint8)((implParam->c_fact >> 8) & 0x000000FF);
    SaveFncDataBuffer[14] = (uint8)((implParam->c_fact >> 16) & 0x000000FF);
    SaveFncDataBuffer[15] = (uint8)((implParam->c_fact >> 24) & 0x000000FF);
    SaveFncDataBuffer[16] = (uint8)implParam->sfrA;

    /** - Execute save function of block */
    error = AdaptiveNotch_FiP32_Save(block, SaveFncDataBuffer, 17);
