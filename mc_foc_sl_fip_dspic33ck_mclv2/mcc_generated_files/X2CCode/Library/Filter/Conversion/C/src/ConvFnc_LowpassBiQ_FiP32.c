...
#include "ConvFnc_LowpassBiQ_FiP32.h"
...
/** Private prototypes */
static uint8 convertM2I(const LOWPASSBIQ_FIP32_MASK_PARAM *maskParam, LOWPASSBIQ_FIP32_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    uint8 sfrb0, sfrb1, sfrb2, sfra1, sfra2;
    ...
    sfrb0 = getQFormat(b1, 32);
    sfrb1 = getQFormat(b1, 32);
    sfrb2 = getQFormat(b2, 32);
    impParam->sfrb0 = sfrb0;
    impParam->sfrb1 = sfrb1;
    impParam->sfrb2 = sfrb2;

    /* determine denominator shift factor */
    sfra1 = getQFormat(a1, 32);
    sfra2 = getQFormat(a2, 32);
    impParam->sfra1 = sfra1;
    impParam->sfra2 = sfra2;

    /* set coefficients */
    impParam->b0 = getQx32Value(b0, sfrb0);
    impParam->b1 = getQx32Value(b1, sfrb1);
    impParam->b2 = getQx32Value(b2, sfrb2);
    impParam->a1 = getQx32Value(a1, sfra1);
    impParam->a2 = getQx32Value(a2, sfra2);
 ...
 */
uint8 LowpassBiQ_FiP32_LoadMP(const LOWPASSBIQ_FIP32_MASK_PARAM *maskParam, uint8 data[], uint16* dataLen, uint16 maxSize)
{
    uint8 error = (uint8)0;
 ...
 */
uint8 LowpassBiQ_FiP32_SaveMP(LOWPASSBIQ_FIP32 *block, LOWPASSBIQ_FIP32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    uint8 error = (uint8)0;
    ...
    else
    {
        ...
        LowpassBiQ_FiP32_BackupMP(maskParam);
        ...
        error = LowpassBiQ_FiP32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            LowpassBiQ_FiP32_RestoreMP(maskParam);
        }
 ...
 */
uint8 LowpassBiQ_FiP32_InitMP(LOWPASSBIQ_FIP32 *block, const LOWPASSBIQ_FIP32_MASK_PARAM *maskParam)
{
    uint8 error = (uint8)0;
    float_CoT block_ts;

    LOWPASSBIQ_FIP32_IMPL_PARAM *implParam = (LOWPASSBIQ_FIP32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = LowpassBiQ_FiP32_Save(block, SaveFncDataBuffer, 25);
 ...
 */
uint8 LowpassBiQ_FiP32_ConvertMP(LOWPASSBIQ_FIP32 *block, LOWPASSBIQ_FIP32_MASK_PARAM *maskParam)
{
    uint8 error;
    float_CoT block_ts;
    LOWPASSBIQ_FIP32_IMPL_PARAM *implParam = (LOWPASSBIQ_FIP32_IMPL_PARAM*)ImplParamBuffer;
 ...
 * @return Nothing
 */
void LowpassBiQ_FiP32_BackupMP(LOWPASSBIQ_FIP32_MASK_PARAM* maskParam)
{
 ...
 * @return Nothing
 */
void LowpassBiQ_FiP32_RestoreMP(LOWPASSBIQ_FIP32_MASK_PARAM* maskParam)
{
