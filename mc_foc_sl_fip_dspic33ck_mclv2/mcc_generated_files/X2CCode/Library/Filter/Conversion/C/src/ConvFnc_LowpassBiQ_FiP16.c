...
#include "ConvFnc_LowpassBiQ_FiP16.h"
...
/** Private prototypes */
static uint8 convertM2I(const LOWPASSBIQ_FIP16_MASK_PARAM *maskParam, LOWPASSBIQ_FIP16_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    sfrb0 = getQFormat(b1, 16);
    sfrb1 = getQFormat(b1, 16);
    sfrb2 = getQFormat(b2, 16);
    impParam->sfrb0 = sfrb0;
    impParam->sfrb1 = sfrb1;
    impParam->sfrb2 = sfrb2;

    /* determine denominator shift factor */
    sfra1 = getQFormat(a1, 16);
    sfra2 = getQFormat(a2, 16);
...
uint8 LowpassBiQ_FiP16_SaveMP(LOWPASSBIQ_FIP16 *block, LOWPASSBIQ_FIP16_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        LowpassBiQ_FiP16_BackupMP(maskParam);
        ...
        error = LowpassBiQ_FiP16_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            LowpassBiQ_FiP16_RestoreMP(maskParam);
        }
...
uint8 LowpassBiQ_FiP16_InitMP(LOWPASSBIQ_FIP16 *block, const LOWPASSBIQ_FIP16_MASK_PARAM *maskParam)
{
    ...
    LOWPASSBIQ_FIP16_IMPL_PARAM *implParam = (LOWPASSBIQ_FIP16_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = LowpassBiQ_FiP16_Save(block, SaveFncDataBuffer, 15);
