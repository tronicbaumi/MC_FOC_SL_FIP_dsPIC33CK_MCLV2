...
#include "ConvFnc_LowpassBiQ_Float64.h"
...
/** Private prototypes */
static uint8 convertM2I(const LOWPASSBIQ_FLOAT64_MASK_PARAM *maskParam, LOWPASSBIQ_FLOAT64_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 LowpassBiQ_Float64_SaveMP(LOWPASSBIQ_FLOAT64 *block, LOWPASSBIQ_FLOAT64_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        LowpassBiQ_Float64_BackupMP(maskParam);
        ...
        error = LowpassBiQ_Float64_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            LowpassBiQ_Float64_RestoreMP(maskParam);
        }
...
uint8 LowpassBiQ_Float64_InitMP(LOWPASSBIQ_FLOAT64 *block, const LOWPASSBIQ_FLOAT64_MASK_PARAM *maskParam)
{
    ...
    LOWPASSBIQ_FLOAT64_IMPL_PARAM *implParam = (LOWPASSBIQ_FLOAT64_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = LowpassBiQ_Float64_Save(block, SaveFncDataBuffer, 40);
