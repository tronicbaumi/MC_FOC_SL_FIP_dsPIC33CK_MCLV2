...
#include "ConvFnc_BandpassBiQ_Float64.h"
...
typedef struct {
    ...
    float64 a1;
    float64 a2;
} BANDPASSBIQ_FLOAT64_IMPL_PARAM;
...
/** Private prototypes */
static uint8 convertM2I(const BANDPASSBIQ_FLOAT64_MASK_PARAM *maskParam, BANDPASSBIQ_FLOAT64_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 BandpassBiQ_Float64_SaveMP(BANDPASSBIQ_FLOAT64 *block, BANDPASSBIQ_FLOAT64_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        BandpassBiQ_Float64_BackupMP(maskParam);
        ...
        error = BandpassBiQ_Float64_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            BandpassBiQ_Float64_RestoreMP(maskParam);
        }
...
uint8 BandpassBiQ_Float64_InitMP(BANDPASSBIQ_FLOAT64 *block, const BANDPASSBIQ_FLOAT64_MASK_PARAM *maskParam)
{
    ...
    BANDPASSBIQ_FLOAT64_IMPL_PARAM *implParam = (BANDPASSBIQ_FLOAT64_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = BandpassBiQ_Float64_Save(block, SaveFncDataBuffer, 40);
