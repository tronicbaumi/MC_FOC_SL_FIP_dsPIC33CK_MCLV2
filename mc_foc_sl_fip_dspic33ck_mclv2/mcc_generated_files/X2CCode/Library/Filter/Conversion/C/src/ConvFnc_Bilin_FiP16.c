...
typedef struct {
    ...
    int8 sfrb1;
    int8 sfra1;
} BILIN_FIP16_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const BILIN_FIP16_MASK_PARAM *maskParam, BILIN_FIP16_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    else
    {
        ...
        impParam->b0 = getQx16Value(b0, sfrb0);
        impParam->b1 = getQx16Value(b1, sfrb1);
        impParam->a1 = getQx16Value(a1, sfra1);
    }
...
uint8 Bilin_FiP16_SaveMP(BILIN_FIP16 *block, BILIN_FIP16_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        Bilin_FiP16_BackupMP(maskParam);
...
uint8 Bilin_FiP16_InitMP(BILIN_FIP16 *block, const BILIN_FIP16_MASK_PARAM *maskParam)
{
    ...
    BILIN_FIP16_IMPL_PARAM *implParam = (BILIN_FIP16_IMPL_PARAM*)ImplParamBuffer;
    ...
    SaveFncDataBuffer[4] = (uint8)(implParam->a1 & 0x00FF);
    SaveFncDataBuffer[5] = (uint8)((implParam->a1 >> 8) & 0x00FF);
    SaveFncDataBuffer[6] = (uint8)implParam->sfrb0;
    SaveFncDataBuffer[7] = (uint8)implParam->sfrb1;
    SaveFncDataBuffer[8] = (uint8)implParam->sfra1;

    /** - Execute save function of block */
    error = Bilin_FiP16_Save(block, SaveFncDataBuffer, 9);
