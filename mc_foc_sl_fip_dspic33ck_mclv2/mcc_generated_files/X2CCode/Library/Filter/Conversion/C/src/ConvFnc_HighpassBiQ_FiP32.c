...
#include "ConvFnc_HighpassBiQ_FiP32.h"
...
/** Private prototypes */
static uint8 convertM2I(const HIGHPASSBIQ_FIP32_MASK_PARAM *maskParam, HIGHPASSBIQ_FIP32_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 HighpassBiQ_FiP32_SaveMP(HIGHPASSBIQ_FIP32 *block, HIGHPASSBIQ_FIP32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        HighpassBiQ_FiP32_BackupMP(maskParam);
        ...
        error = HighpassBiQ_FiP32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            HighpassBiQ_FiP32_RestoreMP(maskParam);
        }
...
uint8 HighpassBiQ_FiP32_InitMP(HIGHPASSBIQ_FIP32 *block, const HIGHPASSBIQ_FIP32_MASK_PARAM *maskParam)
{
    ...
    HIGHPASSBIQ_FIP32_IMPL_PARAM *implParam = (HIGHPASSBIQ_FIP32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = HighpassBiQ_FiP32_Save(block, SaveFncDataBuffer, 25);
