...
#include "ConvFnc_HighpassBiQ_Float32.h"
...
typedef struct {
    ...
    float32 a1;
    float32 a2;
} HIGHPASSBIQ_FLOAT32_IMPL_PARAM;
...
/** Private prototypes */
static uint8 convertM2I(const HIGHPASSBIQ_FLOAT32_MASK_PARAM *maskParam, HIGHPASSBIQ_FLOAT32_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 HighpassBiQ_Float32_SaveMP(HIGHPASSBIQ_FLOAT32 *block, HIGHPASSBIQ_FLOAT32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        HighpassBiQ_Float32_BackupMP(maskParam);
        ...
        error = HighpassBiQ_Float32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            HighpassBiQ_Float32_RestoreMP(maskParam);
        }
...
uint8 HighpassBiQ_Float32_InitMP(HIGHPASSBIQ_FLOAT32 *block, const HIGHPASSBIQ_FLOAT32_MASK_PARAM *maskParam)
{
    ...
    HIGHPASSBIQ_FLOAT32_IMPL_PARAM *implParam = (HIGHPASSBIQ_FLOAT32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = HighpassBiQ_Float32_Save(block, SaveFncDataBuffer, 20);
