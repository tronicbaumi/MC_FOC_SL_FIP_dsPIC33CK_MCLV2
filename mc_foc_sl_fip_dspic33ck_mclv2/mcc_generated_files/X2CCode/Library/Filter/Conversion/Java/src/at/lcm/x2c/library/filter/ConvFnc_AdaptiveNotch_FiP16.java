...
public class ConvFnc_AdaptiveNotch_FiP16 extends JavaConversionFunction {
...
// USERCODE-BEGIN:ConvMaskToImplementation                                                                              
        ...
        double n_max = Math.abs(Double.valueOf(n_maxMaskVal.getValue())); /* speed scaling */
        ...
        if ((((2 * n_max) / 60) * p) > (1 / Ts)) {
            throw new Exception("Nyquist criteria is violated -> decrease n_max or Ts*ts_fact!");
        }

        /* calculate multiplication factors for filter coefficients */
        double a_fact = ((((-2 * n_max * Math.PI) / 60) * p) / 2 / Q) * Ts;
        double b_fact = ((((-2 * n_max) / 60) * p) / 2 / Q) * Ts * Math.sqrt((4 * Q * Q) - 1);
        double c_fact = (((2 * n_max) / 60) * p) * Ts;

        int sfrA = QFormat.getQFormat(a_fact, BITS, true);
        /* B and C factors are always smaller than 1 -> no shift factor calculation needed */

        /* set implementation parameter values */
        n_threshCtrVal.setReal(0, 0, QFormat.getQValue(n_thresh / n_max, BITS - 1, BITS, true));
        sfrACtrVal.setReal(0, 0, sfrA);
        a_factCtrVal.setReal(0, 0, QFormat.getQValue(a_fact, sfrA, BITS, true));
        b_factCtrVal.setReal(0, 0, QFormat.getQValue(b_fact, BITS - 1, BITS, true));
        c_factCtrVal.setReal(0, 0, QFormat.getQValue(c_fact, BITS - 1, BITS, true));
...
// USERCODE-BEGIN:ConvImplementationToMask                                                                              
        if (true) {
            throw new Exception("Implementation-to-Mask conversion not implemented for Block 'AdaptiveNotch', Implementation 'FiP16' in Library 'Filter'");
        }
