...
public class ConvFnc_BandstopBiQ_FiP32 extends JavaConversionFunction {
...
// USERCODE-BEGIN:ConvMaskToImplementation                                                                              
        ...
        /* -------------- Get analog lowpass prototype with function */
        double z[];
        double p_x = 0;
        double p_y = 0;
        double k = 0;
        if ((characteristicMaskVal.getValue().equals("Butterworth"))
            ...
            if (rp == 0) {
                throw new Exception("Division by zero! Choose maximum passband ripple different from zero!");
            }
        ...
        double p_hp_x = ((bw / 2) * p_x) / scale_p;
        double p_hp_y = 0; /* p_hp_y = bw/2 * p_y/scale_p; */

        /* zeros */
        double z_bs_x = 0;
        double z_bs_y = wo;
        /* poles */
        double p_bs_x = p_hp_x;
        double arg = (p_hp_x * p_hp_x) - (wo * wo);
        double p_bs_y = Math.sqrt(Math.abs(arg));
        /* gain */
        double prod_z = 1; // analog lowpass prototype has no zeros
        double prod_p = -p_x; // analog lowpass prototype (N=1) has only one real pole.
        double k_bs = (k * prod_z) / prod_p;

        /* -------------- transform to discrete bandstop (bilinear transform) */
        double fs2 = 2.0 * 2;
        ...
        /* zeros */
        double nom_x = fs2 + z_bs_x;
        double nom_y = z_bs_y;
        double denom_x = fs2 - z_bs_x;
        double denom_y = -z_bs_y;
        ...
        prod_z = (denom_x * denom_x) + (denom_y * denom_y);
        ...
        prod_p = (denom_x * denom_x) + (denom_y * denom_y); /* product of conj complex number */
        double kz = (k_bs * prod_z) / prod_p;

        /* -------------- transform to proper form (=ba) */
        /* numerator coefficients */
        double b0 = kz * 1; /* coefficient to s^2 */
        double b1 = kz * -2 * zz_x; /* coefficient to s^1 */
        double b2 = (kz * Math.pow(zz_x, 2)) + Math.pow(zz_y, 2); /* coefficient to s^0 */
...
// USERCODE-BEGIN:ConvImplementationToMask                                                                              
        if(true){
            throw new Exception("Implementation-to-Mask conversion not implemented for Block 'BandstopBiQ', Implementation 'FiP32' in Library 'Filter' due to ambiguous parameters");
        }
